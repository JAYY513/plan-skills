...
4. **出现调研需求** → 登记前先查已有结论（FINDINGS.md 索引、docs/01-wiki、docs/research、个人知识库），命中则直接引用既有条目、不重复调研；未命中才进 TASKS.md「调研（限时探针）」区，必须向用户确认时间盒和产出要求（产出默认要求落 FINDINGS.md），缺了就问
