---
name: plan-task
description: 任务从讨论到完成的全生命周期管理（自动驾驶）。当对话中出现"加个任务""把 xxx 加进计划""帮我拆一下这个任务""下一步做什么""现在什么进度 / 当前状态 / status""开工 / 开始做 X""任务做完了""记录一个新想法"，或在讨论实施计划、拆任务、调研结论、失败尝试时使用——这些信号出现时自动落盘，不依赖用户点名技能。负责分流判断（TASKS / INBOX / FINDINGS）、控制 0.5~2 天粒度、生成 DoD、认领开工时自动建 .planning/ 工作区、执行 2-Action 落盘纪律、完成时先走对齐门再三合一并标 ✅。曾用名：new-task、task-plan（两技能已合并为本技能）。
hooks:
  SessionStart:
    - hooks:
        - type: command
...
2. **对齐门**（调 `finish` 之前必做）。以 **DoD 实测** 为准，不以 `plan.md` 原计划为准。对不上 / 拿不准未决 / 红线未确认 → 拒绝 `finish`，先改 FINDINGS 或问一句。
   - **对应**：FINDINGS 结论必须对得上实测。中途改了实现 → 改结论或追加「补」，禁止旧结论入库
   - **覆盖**：重读 progress.md 日志「决策 / 错误」（无工作区则扫本会话）。每条要么进 FINDINGS（新条或「补」），要么在该条旁标明「不晋升结论」
...
3. 若该任务有 `.planning/` 工作区 → agent 先写好三合一的内容部分（见下）
4. 调引擎完成校验与机械动作：
...
5. **不许删除**——归档是 plan-review 的职责
6. 检查队列水位，不足则提议补充
7. 向用户输出对齐卡（冲突行三选一；红线未确认走「已停车」且本步未调 `finish`）：
...
1. **结论回填 FINDINGS.md**：按条目格式写入（无新结论则写存根），并加一行引用：`过程追溯：.planning/done/<slug>/progress.md`（至少一条带此行）
