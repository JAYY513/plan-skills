...
# 校验条件与 task-plan 技能的「完成三合一动作」一致：存在活跃工作区但对应任务未标 ✅
...
if ($warn.Count -gt 0) {
  ...
  Write-Output "[plan] 若任务已完成，请先执行三合一动作（结论回填 FINDINGS.md + progress.md 顶部固化 postmortem + 工作区移入 .planning/done/），再走 new-task 标 ✅；若任务未完成，请在 progress.md 记录当前进展供下次恢复。"
}
